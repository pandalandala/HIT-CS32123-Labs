...
public class TurtleSoup {
     ...
     *         must be 0 <= angle < 360
     */
    public static double calculateBearingToPoint(double currentBearing, int currentX, int currentY, int nextX, int nextY) {
        double angle = Math.atan2(nextY - currentY, nextX - currentX) * 180.0 / Math.PI;
        if (angle < 0)
            angle += 360.0;
    ...
    public static List<Double> calculateBearings(List<Integer> xCoords, List<Integer> yCoords) {
        ...
        int currentX = xCoords.get(0), currentY = yCoords.get(0), destX, destY;
        int length = xCoords.size();
        for (int i = 1; i < length; i++) {
            destX = xCoords.get(i);
            destY = yCoords.get(i);
            bearings.add(calculateBearingToPoint(currentBearing, currentX, currentY, destX, destY));
    ...
    public static void drawPersonalArt(Turtle turtle) {
        int Size = 200, Step = 1, Densi = 1, ColorNum = 6;
    ...
    public static void main(String args[]) {
        ...
        //drawRegularPolygon(turtle, 17, 70);
